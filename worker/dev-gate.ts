...
function gatePage(next: string, status = 200, failed = false) {
  ...
  const error = failed
    ...
    : "";
  return new Response(
    `<!doctype html><html lang="en"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Zoomigo preview</title><style>body{font-family:system-ui;background:#f4f1e8;color:#17211b;margin:0}main{max-width:26rem;margin:12vh auto;padding:2rem}form{display:grid;gap:1rem}.password-row{display:flex;gap:.5rem}.password-row input{min-width:0;flex:1}input,button{font:inherit;padding:.8rem;border:1px solid #8c978f;border-radius:.5rem}button{background:#173f35;color:white;border-color:#173f35}.password-row button{background:white;color:#173f35;white-space:nowrap}</style><main><h1>Zoomigo preview</h1><p>Enter the shared preview password.</p>${error}<form method="post" action="/_dev-gate"><input id="preview-next" type="hidden" name="next" value="${escapedNext}"><label for="preview-password">Password</label><div class="password-row"><input id="preview-password" name="password" type="password" required autocomplete="current-password"><button id="preview-password-toggle" type="button" aria-controls="preview-password" aria-pressed="false">Show password</button></div><button type="submit">Continue</button></form></main><script nonce="${nonce}">const next=document.getElementById("preview-next");if(location.hash&&next.value.startsWith("/login")){next.value+=location.hash}const password=document.getElementById("preview-password");const toggle=document.getElementById("preview-password-toggle");toggle.addEventListener("click",()=>{if(password.type==="password"){password.type="text";toggle.textContent="Hide password";toggle.setAttribute("aria-pressed","true")}else{password.type="password";toggle.textContent="Show password";toggle.setAttribute("aria-pressed","false")}})</script></html>`,
    {
      status,
      headers: {

...
const expectedRelease = requiredEnvironment("DEV_SMOKE_EXPECTED_RELEASE");
const readOnly = process.argv.includes("--read-only");

const access = await proveContainer();
if (readOnly) {
  console.log(`exact dev container is serving ${expectedRelease}`);
} else {
  await proveFinalFlows(access);
}
  ...
  for (let attempt = 1; attempt <= 12; attempt += 1) {
    try {
      ...
      assert.equal(
        ready.release,
        expectedRelease,
        "the requested release is not serving traffic",
      );
...
async function proveFinalFlows(access) {
  ...
  assert.equal(
    rest.planId,
    plan.id,
    "planned-rest check-in used the wrong plan",
  );

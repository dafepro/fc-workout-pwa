...
export function TrainingPlanBuilder({ teamId }: { teamId: string }) {
  ...
  const planDays = draftDays.length ? draftDays : (template?.days ?? []);
  ...
  function updateDay(index: number, update: Partial<TrainingPlanDay>) {
    setDraftDays((current) => {
      const source = current.length ? current : editablePlanDays(template.days);
                        ...
                        onChange={(event) => {
                          const block = activityChoices.find(
                            ({ activityDefinitionId }) =>
                              activityDefinitionId === event.target.value,
                          );
                          if (block) {
                            updateDay(index, {
                              blocks: [
...
function normalizeEditedDay(day: TrainingPlanDay): TrainingPlanDay {
  ...
  const duration = day.durationMinutes || 15;
  return {
    ...day,
...
function planStatusLabel(plan: TrainingPlan): string {
  const labels = consoleCopy.trainingPlans.status;

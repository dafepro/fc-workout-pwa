...
allowed_key() {
	case "$1" in
		STAFF_SECRET_KEY|PLAYER_LOGIN_URL|STAFF_SETUP_URL|PRODUCTION_DATA_APPROVED|REWARD_MAILER_MODE|REWARD_EMAIL_FROM|REWARD_EMAIL_BASE_URL|RESEND_API_KEY|ENABLE_OBSERVABILITY|OBSERVABILITY_DATA_DIR|GRAFANA_LOGS_URL|GRAFANA_LOGS_USERNAME|GRAFANA_LOGS_TOKEN|GRAFANA_METRICS_URL|GRAFANA_METRICS_USERNAME|GRAFANA_METRICS_TOKEN) return 0 ;;
		*) return 1 ;;
	esac
}

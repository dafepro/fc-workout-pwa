...
# This must finish before the checkout changes the scripts beneath systemd.
run_ssh "sudo -n systemctl start zoomigo-backup.service"

image="ghcr.io/dafepro/fc-workout-pwa/api:sha-$release_sha"
run_ssh "set -eu; cd /opt/app; test -z \"\$(git status --porcelain --untracked-files=no)\"; git fetch --depth=1 origin '$release_sha'; git checkout --detach '$release_sha'; cd deploy/vm; sudo -n ./scripts/set-release.sh .env '$image' '$release_sha'; sudo -n ./scripts/prepare-host.sh .env; ./scripts/preflight.sh .env; ./scripts/deploy.sh .env; sudo -n ./scripts/install-backup-service.sh; sudo -n ./scripts/production-check.sh .env --check-s3"

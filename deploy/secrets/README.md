...
| `RESEND_API_KEY`                                          | release                 | Sending-only Resend credential for Team Reward close and achieved notices. Leave absent while `REWARD_MAILER_MODE=sink`.                                                                                                                    |
...
| `REWARD_MAILER_MODE` / `REWARD_EMAIL_FROM` / `REWARD_EMAIL_BASE_URL`                  | Team Reward email adapter (`sink` until launch, then `resend`), verified sender on the dedicated notify subdomain, and the public PWA origin used in staff links.                                                                                                    |

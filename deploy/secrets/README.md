...
| `STAFF_CONSOLE_EMAIL_ADDRESSES` / `STAFF_CONSOLE_TEAM_DOMAIN`                         | OpenTofu inputs for the Cloudflare Access gate: the named addresses admitted to `/staff/*`, and the Zero Trust team domain label to create. Leave the team domain empty on an account that already has an organization.                                              |

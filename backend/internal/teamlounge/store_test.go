...
func TestPlacementBudgetBackfillsCurrentWeekCheckInsAndUsesTeamTime(t *testing.T) {
	...
	budget, err := store.PlacementBudget(t.Context(), "team:team-one:lounge:v21", "player-one", now)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlacementBudgetDoesNotChargeRetiredRoomPlacementsToTheActiveRoom(t *testing.T) {
	...
	if _, err := store.PlacementBudget(t.Context(), "team:team-one:lounge:v21", "player-one", now); err != nil {
		t.Fatal(err)
	}
	for _, statement := range []string{
	...
	reservation, err := store.ReservePlacement(t.Context(), "team:team-one:lounge:v21", "player-one", "active-room-placement",
		PlacementRequest{DefinitionID: "zoomigo-stamp-bolt", DefinitionVersion: 3, X: 45, Y: 75}, now)
...
func TestReservePlacementConsumesOneCreditIdempotently(t *testing.T) {
	...
	store := NewSQLiteStore(db, BeachBoardwalkLoungeCatalog())
	now := time.Date(2026, time.August, 26, 18, 0, 0, 0, time.UTC)
	request := PlacementRequest{DefinitionID: "zoomigo-stamp-bolt", DefinitionVersion: 3, X: 40, Y: 70}
	if _, err := store.ReservePlacement(t.Context(), "team:team-one:lounge:v21", "player-one", "unbound-room", request, now); !errors.Is(err, ErrPlacementUnavailable) {
		t.Fatalf("unbound room reservation error = %v", err)
	}
	bindPlacementRoom(t, store)
	first, err := store.ReservePlacement(t.Context(), "team:team-one:lounge:v21", "player-one", "one-request", request, now)
	if err != nil || first.Replayed || first.Remaining != 0 || first.ID == "" {
		t.Fatalf("first reservation = %+v, %v", first, err)
	}
	replay, err := store.ReservePlacement(t.Context(), "team:team-one:lounge:v21", "player-one", "one-request", request, now)
	if err != nil || !replay.Replayed || replay.ID != first.ID {
		t.Fatalf("replayed reservation = %+v, %v", replay, err)
	}
	store.now = func() time.Time { return now }
	authorization := authorizationRequest(replay, "player-one", "team:team-one:lounge:v21", "mutation-one")
	...
	outcome := roomsdk.MutationOutcome{
		...
		RoomID: "team:team-one:lounge:v21", ParticipantID: "player-one",
	...
	_, err = store.ReservePlacement(t.Context(), "team:team-one:lounge:v21", "player-one", "another-request", request, now)
...
func TestReservePlacementRequiresOwnedEarnedItem(t *testing.T) {
	...
	bindPlacementRoom(t, store)
	now := time.Date(2026, time.August, 26, 18, 0, 0, 0, time.UTC)
	request := PlacementRequest{DefinitionID: "zoomigo-prop-play-hammock", DefinitionVersion: 5, X: 40, Y: 70, Scale: 1.4}
	if _, err := store.ReservePlacement(t.Context(), "team:team-one:lounge:v21", "player-one", "locked-item", request, now); !errors.Is(err, ErrPlacementItemUnavailable) {
		t.Fatalf("unowned reservation error = %v", err)
	}
	if _, err := db.ExecContext(t.Context(), `INSERT INTO player_unlocks (player_id, item_kind, item_id, source, unlocked_at) VALUES ('player-one', 'lounge_prop', 'lounge-prop-hammock', 'daily_check_in', '2026-08-26T17:30:00Z')`); err != nil {
		t.Fatal(err)
	}
	if reservation, err := store.ReservePlacement(t.Context(), "team:team-one:lounge:v21", "player-one", "owned-item", request, now); err != nil || reservation.Scale != float64(float32(1.4)) {
		t.Fatalf("owned reservation = %+v, %v", reservation, err)
	}
}
...
func TestReservePlacementSerializesConcurrentCreditClaims(t *testing.T) {
	...
	for index := range 8 {
		...
		go func() {
			defer wait.Done()
			<-start
			_, err := store.ReservePlacement(t.Context(), "team:team-one:lounge:v21", "player-one", fmt.Sprintf("claim-%d", index), request, now)
			errorsSeen <- err
		}()
	}
	close(start)
...
func TestLoadSnapshotNormalizesCollisionProneGeneratedItemIDs(t *testing.T) {
	...
	seedTeam(t, db)
	store := NewSQLiteStore(db, Catalog{})
	roomID := "team:team-one:lounge:v21"
	...
	for _, statement := range []string{
		...
		`INSERT INTO team_lounge_placement_reservations (
		 ...
		 ('reservation-wobble', 'team-one', 'player-one', '2026-08-24', '2026-08-30', 'team:team-one:lounge:v21', 'beach-boardwalk', 21,
		 ...
		 ('reservation-rocket', 'team-one', 'player-one', '2026-08-24', '2026-08-30', 'team:team-one:lounge:v21', 'beach-boardwalk', 21,
		  'zoomigo-stamp-rocket', 3, 22, 40, 0, 1, '{}', randomblob(32), randomblob(32), randomblob(32), '2026-08-30T12:05:00Z', 'committed', 'i1', '2026-08-30T12:00:02Z', '2026-08-30T12:00:03Z'),
		 ('reservation-rocket-two', 'team-one', 'player-one', '2026-08-24', '2026-08-30', 'team:team-one:lounge:v21', 'beach-boardwalk', 21,
		  'zoomigo-stamp-rocket', 3, 23, 40, 0, 1, '{}', randomblob(32), randomblob(32), randomblob(32), '2026-08-30T12:05:00Z', 'committed', 'i1', '2026-08-30T12:00:04Z', '2026-08-30T12:00:05Z')`,
	} {
		if _, err := db.ExecContext(t.Context(), statement); err != nil {
			t.Fatal(err)
		}
	...
	snapshot := roomsdk.CanvasSnapshot{
		...
		Items: []roomsdk.SnapshotItem{
			{EntityID: "boardwalk-beach-ball", DefinitionID: "beach-ball", DefinitionVersion: 9, ItemRevision: 1},
			{EntityID: "i1", DefinitionID: "zoomigo-prop-play-wobble-cone", DefinitionVersion: 3, OwnerUserID: "player-one", ItemRevision: 1},
			{EntityID: "i1", DefinitionID: "zoomigo-stamp-rocket", DefinitionVersion: 3, OwnerUserID: "player-one", ItemRevision: 1},
			{EntityID: "i1", DefinitionID: "zoomigo-stamp-rocket", DefinitionVersion: 3, OwnerUserID: "player-one", ItemRevision: 1},
			{EntityID: "safe-item", DefinitionID: "zoomigo-stamp-bolt", DefinitionVersion: 3, OwnerUserID: "player-one", ItemRevision: 1},
		},
	}
...
func TestBoundRoomStoreKeepsSnapshotAcrossWeekRollover(t *testing.T) {
	...
	unbound := "team:team-two:lounge:v21"

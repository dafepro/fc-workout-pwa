...
func TestPlacementBudgetBackfillsCurrentWeekCheckInsAndUsesTeamTime(t *testing.T) {
	...
	budget, err := store.PlacementBudget(t.Context(), "team:team-one:lounge:v20", "player-one", now)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlacementBudgetDoesNotChargeRetiredRoomPlacementsToTheActiveRoom(t *testing.T) {
	...
	if _, err := store.PlacementBudget(t.Context(), "team:team-one:lounge:v20", "player-one", now); err != nil {
		t.Fatal(err)
	}
	for _, statement := range []string{
	...
	reservation, err := store.ReservePlacement(t.Context(), "team:team-one:lounge:v20", "player-one", "active-room-placement",
...
func TestReservePlacementConsumesOneCreditIdempotently(t *testing.T) {
	...
	if _, err := store.ReservePlacement(t.Context(), "team:team-one:lounge:v20", "player-one", "unbound-room", request, now); !errors.Is(err, ErrPlacementUnavailable) {
		t.Fatalf("unbound room reservation error = %v", err)
	}
	bindPlacementRoom(t, store)
	first, err := store.ReservePlacement(t.Context(), "team:team-one:lounge:v20", "player-one", "one-request", request, now)
	if err != nil || first.Replayed || first.Remaining != 0 || first.ID == "" {
		t.Fatalf("first reservation = %+v, %v", first, err)
	}
	replay, err := store.ReservePlacement(t.Context(), "team:team-one:lounge:v20", "player-one", "one-request", request, now)
	if err != nil || !replay.Replayed || replay.ID != first.ID {
		t.Fatalf("replayed reservation = %+v, %v", replay, err)
	}
	store.now = func() time.Time { return now }
	authorization := authorizationRequest(replay, "player-one", "team:team-one:lounge:v20", "mutation-one")
	...
	outcome := roomsdk.MutationOutcome{
		...
		RoomID: "team:team-one:lounge:v20", ParticipantID: "player-one",
	...
	_, err = store.ReservePlacement(t.Context(), "team:team-one:lounge:v20", "player-one", "another-request", request, now)
...
func TestReservePlacementRequiresOwnedEarnedItem(t *testing.T) {
	...
	if _, err := store.ReservePlacement(t.Context(), "team:team-one:lounge:v20", "player-one", "locked-item", request, now); !errors.Is(err, ErrPlacementItemUnavailable) {
	...
	if reservation, err := store.ReservePlacement(t.Context(), "team:team-one:lounge:v20", "player-one", "owned-item", request, now); err != nil || reservation.Scale != float64(float32(1.4)) {
...
func TestReservePlacementSerializesConcurrentCreditClaims(t *testing.T) {
	...
	for index := range 8 {
		...
		go func() {
			defer wait.Done()
			<-start
			_, err := store.ReservePlacement(t.Context(), "team:team-one:lounge:v20", "player-one", fmt.Sprintf("claim-%d", index), request, now)
			errorsSeen <- err
		}()
	}
	close(start)
...
func TestLoadSnapshotNormalizesCollisionProneGeneratedItemIDs(t *testing.T) {
	...
	seedTeam(t, db)
	store := NewSQLiteStore(db, Catalog{})
	roomID := "team:team-one:lounge:v20"
	...
	for _, statement := range []string{
		...
		`INSERT INTO team_lounge_placement_reservations (
		 ...
		 ('reservation-wobble', 'team-one', 'player-one', '2026-08-24', '2026-08-30', 'team:team-one:lounge:v20', 'beach-boardwalk', 20,
		 ...
		 ('reservation-rocket', 'team-one', 'player-one', '2026-08-24', '2026-08-30', 'team:team-one:lounge:v20', 'beach-boardwalk', 20,
		 ...
		 ('reservation-rocket-two', 'team-one', 'player-one', '2026-08-24', '2026-08-30', 'team:team-one:lounge:v20', 'beach-boardwalk', 20,
...
func TestBoundRoomStoreKeepsSnapshotAcrossWeekRollover(t *testing.T) {
	...
	unbound := "team:team-two:lounge:v20"

...
func TestTeamLoungeAuthenticatorConformsAndConsumesOneTimeTickets(t *testing.T) {
	...
	roomsdktest.RunAuthenticatorConformance(t, auth, []roomsdktest.AuthenticatorCase{
		{
			Name: "team member with exact room ticket", Request: request("team:team-one:lounge:2026-08-24:v5", valid),
			WantIdentity: roomsdk.Identity{UserID: "player-one", DisplayName: "Player"},
		},
		{Name: "missing credential", Request: request("team:team-one:lounge:2026-08-24:v5", ""), Unauthorized: true},
		{Name: "V1 credential", Request: request("team:team-one:lounge:2026-08-24:v5", v1), Unauthorized: true},
	})
	if visits.visits != 1 || visits.roomID != "team:team-one:lounge:2026-08-24:v5" || visits.playerID != "player-one" {
		t.Fatalf("recorded visits = %#v", visits)
	}

	if identity, err := auth.Authenticate(t.Context(), request("team:team-one:lounge:2026-08-24:v5", valid)); err == nil || identity != (roomsdk.Identity{}) {

...
type teamRewardMediaRepository interface {
...
func (service *service) getStaffTeamRewardMedia(w http.ResponseWriter, r *http.Request) {
	...
	repository := service.staffStore.(teamRewardMediaRepository)
	media, err := repository.TeamRewardMedia(r.Context(), teamID, r.PathValue("mediaId"))
	if service.writeTeamRewardMediaError(w, r, err) {
		return
	}
...
func (service *service) getPlayerTeamRewardMedia(w http.ResponseWriter, r *http.Request) {
	...
	repository := service.staffStore.(teamRewardMediaRepository)
	media, err := repository.TeamRewardMediaForPlayer(r.Context(), actor, r.PathValue("teamId"), r.PathValue("mediaId"), service.now().UTC())
	if service.writeTeamRewardMediaError(w, r, err) {
		return
	}
	service.serveTeamRewardMedia(w, r, media)
...
func (service *service) rewardMediaReady(w http.ResponseWriter, r *http.Request) bool {
	_, repositoryReady := service.staffStore.(teamRewardMediaRepository)
	if !repositoryReady || service.rewardMedia == nil || service.rewardImages == nil {
...
func (service *service) writeTeamRewardMediaError(w http.ResponseWriter, r *http.Request, err error) bool {
	if err == nil {
		return false
	}
	if errors.Is(err, store.ErrTeamRewardUnavailable) || errors.Is(err, store.ErrStaffNotFound) {
		writeError(w, r, http.StatusNotFound, "not_found", "The requested image was not found.")
		return true
	}
	if errors.Is(err, store.ErrStaffInvalid) {
		writeError(w, r, http.StatusBadRequest, "invalid_reward_media", "The reward image is invalid.")
		return true
	}
	writeError(w, r, http.StatusInternalServerError, "reward_media_failed", "The image could not be loaded.")
	return true
}
...
func (service *service) cleanupUnattachedRewardMedia(r *http.Request, repository teamRewardMediaRepository) {
	now := service.now().UTC()
	_, err := rewardmedia.CleanupExpired(r.Context(), repository, service.rewardMedia, now.Add(-24*time.Hour), now)
	if err != nil {

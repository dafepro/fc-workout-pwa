...
func (store *Store) CreateTeamRewardMedia(ctx context.Context, input CreateTeamRewardMediaInput) (TeamRewardMedia, error) {
	...
	if input.TeamID == "" || input.CreatedByAccountID == "" || !rewardMediaStorageKey.MatchString(input.StorageKey) ||
		...
		return TeamRewardMedia{}, ErrTeamRewardInvalid
	}
	stamp := input.Now.UTC().Format(time.RFC3339Nano)
	media := TeamRewardMedia{
		ID: newID("reward_media"), TeamID: input.TeamID, StorageKey: input.StorageKey,
	...
	_, err := store.db.ExecContext(ctx, `INSERT INTO team_reward_media (
...
func (store *Store) TeamRewardMedia(ctx context.Context, teamID, mediaID string) (TeamRewardMedia, error) {
	media, err := scanTeamRewardMedia(store.db.QueryRowContext(ctx, `SELECT id, team_id, storage_key, sha256,
...
func (store *Store) TeamRewardMediaForPlayer(ctx context.Context, actor domain.Actor, teamID, mediaID string, now time.Time) (TeamRewardMedia, error) {
	reward, err := store.TeamRewardForPlayer(ctx, actor, teamID, now)
	if err != nil || reward.MediaID == "" || reward.MediaID != mediaID {
		return TeamRewardMedia{}, ErrTeamRewardUnavailable
	}
	return store.TeamRewardMedia(ctx, teamID, mediaID)
}

func (store *Store) ExpireUnattachedTeamRewardMedia(ctx context.Context, before, now time.Time) ([]TeamRewardMedia, error) {
	rows, err := store.db.QueryContext(ctx, `SELECT id, team_id, storage_key, sha256, mime_type, width, height,
	...
	for _, media := range candidates {
		result, updateErr := store.db.ExecContext(ctx, `UPDATE team_reward_media SET deleted_at = ?
...
func (store *Store) RestoreExpiredTeamRewardMedia(ctx context.Context, mediaID string) error {
	result, err := store.db.ExecContext(ctx, `UPDATE team_reward_media SET deleted_at = NULL
...
func scanTeamRewardMedia(row rewardRow) (TeamRewardMedia, error) {

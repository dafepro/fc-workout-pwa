...
func (store *Store) ResetE2EFixtures(ctx context.Context, now time.Time) error {
	...
	defer tx.Rollback()
	statements := []string{
		"DELETE FROM plan_prize_box_grants",
		"DELETE FROM daily_drop_claims",
		"DELETE FROM player_unlocks",
		"DELETE FROM team_reward_moderation_events",
		"DELETE FROM team_reward_reports",
		"DELETE FROM team_reward_notification_outbox",
		...
		"DELETE FROM team_lounge_v2_weekly_visits",
		"DELETE FROM team_lounge_v2_snapshots",
		"DELETE FROM team_lounge_v2_room_bindings",
		"DELETE FROM team_lounge_v2_placement_credits",

...
func ValidateTrainingPlanTemplate(template TrainingPlanTemplate) []string {
	...
	for index, day := range template.Days {
		...
		if !validKind || !validFocus || !validIntensity {
			errors = appendPlanError(errors, "Plan focus and intensity must use approved options.")
		}
		if day.Offset != index {
			errors = appendPlanError(errors, "Plan days must be consecutive and start on day zero.")
		}
		if day.Kind == TrainingPlanRest {
			if day.DurationMinutes != 0 || len(day.Blocks) != 0 || day.Focus != "recovery" || day.Intensity != TrainingPlanEasy {
				errors = appendPlanError(errors, "Training days need blocks and rest days must stay empty.")
			}
		} else {
			if day.DurationMinutes < 5 || day.DurationMinutes > 20 {
				errors = appendPlanError(errors, "Active plan days must be between 5 and 20 minutes.")
			}
			if len(day.Blocks) == 0 {
				errors = appendPlanError(errors, "Training days need blocks and rest days must stay empty.")
			}
		...
		for _, block := range day.Blocks {
			...
			if !validActivities[block.ActivityDefinitionID] || block.DurationMinutes <= 0 || block.DurationMinutes > day.DurationMinutes {
				errors = appendPlanError(errors, "Every block must use an approved activity and fit within its day.")
			}
			if (block.ActivityDefinitionID == "timed-run-walk" || block.ActivityDefinitionID == "recovery-walk-jog") && block.DurationMinutes%5 != 0 {
				errors = appendPlanError(errors, "Duration activities must use supported five-minute steps.")
			}
		}
		if totalBlockMinutes > day.DurationMinutes {
			errors = appendPlanError(errors, "The total block time must fit within its day.")
		}
		if day.Kind == TrainingPlanRecovery && (day.Focus != "recovery" || day.Intensity != TrainingPlanEasy || len(day.Blocks) != 1 || day.Blocks[0].ActivityDefinitionID != "recovery-walk-jog") {
			errors = appendPlanError(errors, "Recovery days must stay easy and use recovery activities.")
		}
...
func appendPlanError(values []string, value string) []string {

...
DELETE FROM player_unlocks WHERE item_kind = 'lounge_chat_pack';
UPDATE prize_boxes SET item_kind = NULL, item_id = NULL
  WHERE item_kind = 'lounge_chat_pack';
...
ALTER TABLE prize_boxes RENAME TO prize_boxes_with_chat_packs;
...
SELECT id, player_id, source, daily_day, daily_time_zone, training_plan_id,
  catalog_version, earned_at, earned_idempotency_key_hash, opened_at,
  open_idempotency_key_hash, item_kind, item_id
FROM prize_boxes_with_chat_packs;

DROP TABLE prize_boxes_with_chat_packs;
...
ALTER TABLE player_unlocks RENAME TO player_unlocks_with_chat_packs;
...
FROM player_unlocks_with_chat_packs;

DROP TABLE player_unlocks_with_chat_packs;

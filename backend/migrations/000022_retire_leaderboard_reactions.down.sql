...
ALTER TABLE reactions RENAME TO reactions_without_leaderboard_context;
...
-- Retired rows were deliberately deleted by the up migration and cannot be reconstructed.
...
SELECT id, sender_player_id, recipient_player_id, team_id, reaction_type,
  context_type, context_period, NULL, context_assignment_id,
  team_day, idempotency_key, created_at, read_at, deleted_at,
  remaining_after_send
FROM reactions_without_leaderboard_context;

DROP TABLE reactions_without_leaderboard_context;

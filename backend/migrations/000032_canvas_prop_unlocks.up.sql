...
ALTER TABLE player_unlocks RENAME TO player_unlocks_before_canvas_props;

CREATE TABLE player_unlocks (
  player_id TEXT NOT NULL REFERENCES players(id),
  item_kind TEXT NOT NULL CHECK (item_kind IN ('avatar_part', 'canvas_stamp', 'canvas_prop')),
...
FROM player_unlocks_before_canvas_props;

DROP TABLE player_unlocks_before_canvas_props;
CREATE INDEX player_unlocks_player_new ON player_unlocks(player_id, viewed_at, unlocked_at DESC);

ALTER TABLE daily_drop_claims RENAME TO daily_drop_claims_before_canvas_props;
...
CREATE TABLE daily_drop_claims (
  ...
  item_kind TEXT CHECK (item_kind IN ('avatar_part', 'canvas_stamp', 'canvas_prop')),
...
SELECT * FROM daily_drop_claims_before_canvas_props;

DROP TABLE daily_drop_claims_before_canvas_props;
...
ALTER TABLE plan_prize_box_grants RENAME TO plan_prize_box_grants_before_canvas_props;
...
SELECT * FROM plan_prize_box_grants_before_canvas_props;

DROP TABLE plan_prize_box_grants_before_canvas_props;

...
| 2026-08-26 | 4K      | pending                          | Decouples current-day stamp editing from the placement tray, keeps existing stamps above the new-placement surface, cancels placement when editing, and supplies a dev-only 99-credit test budget | Owner selection without the tray; placement-overlay regression; dev/prod budget-boundary tests; targeted frontend and Go suites                | Direct selection before/after choosing a new stamp; many placements on dev               |

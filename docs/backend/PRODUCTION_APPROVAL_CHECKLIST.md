...
| CI release gate          | Implemented; operator setup required | Static checks and builds run before Docker API/browser E2E and VM smoke; only a passing commit publishes an immutable image. Protected deployment stays disabled until the encrypted bundle, environment review, native host migration, and restore drill are complete. |
| Infrastructure safety    | Implemented; plan/import required    | OpenTofu models the Droplet, restricted firewall, DNS, and secret-free cloud-init. Destructive resources use `prevent_destroy`; CI never applies infrastructure.                                                                                                        |

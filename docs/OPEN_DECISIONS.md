...
- Still open: whether the staff setup token moves from the link's query to its fragment. The Access redirect could not carry a fragment, which is why it is in the query; that constraint is gone and the fragment would keep the token out of edge and proxy logs. Moving it means reissuing every live setup link and changing the operator CLI that mints them, so it is not a free change.

...
No secrets belong in `.tf`, `.tfvars`, cloud-init, or Git. `DIGITALOCEAN_TOKEN` stays in the operator environment; the Cloudflare token and deploy key are read only from the age-encrypted production bundle. `provision.mjs` encrypts local Terraform state back to both public deployment recipients after every operation. Destructive resources use `prevent_destroy`, and there is intentionally no automated destroy command.
